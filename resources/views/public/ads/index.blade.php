...
@section('content')
    ...
    <section class="container py-4">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                <form method="GET" action="{{ route('ads.index') }}" class="row align-items-end" id="filtroForm">
        ...
        @if ($totalResultados > 0 || (!request()->has('q') && !request()->has('categoria_id') && !request()->has('tipo')))
                                            ...
                                            <ul class="ad-meta list-unstyled mb-2">
                                                @if ($ganado->ubicacion)
                                                    <li class="mb-1"><i class="fas fa-map-marker-alt text-success"></i>
                                                        <span
                                                            class="small">{{ Str::limit($ganado->ubicacion, 40) }}</span>
                                                    </li>
                                                @endif
                                                @if ($ganado->tipoAnimal)
                                                    <li class="mb-1"><i class="fas fa-paw text-success"></i> <span
                                                            class="small">{{ $ganado->tipoAnimal->nombre }}</span></li>
                                                @endif
                                                @if ($ganado->edad)
                                                    <li class="mb-1"><i class="fas fa-birthday-cake text-success"></i>
                                                        <span class="small">{{ $ganado->edad }} meses</span>
                                                    </li>
                                                @endif
                                        ...
                                        <div
                                            ...
                                            @if ($ganado->precio)
                                                <span class="price font-weight-bold text-success">Bs
                                                    {{ number_format($ganado->precio, 2) }}</span>
                                            @else
            ...
            @if ($totalResultados == 0 && (request()->has('q') || request()->has('categoria_id') || request()->has('tipo')))
                <div class="alert alert-info text-center py-5">
                    ...
                    <p>Intenta con otros términos de búsqueda o <a href="{{ route('ads.index') }}">ver todos los
                            anuncios</a></p>
                </div>
            @endif
        @else
